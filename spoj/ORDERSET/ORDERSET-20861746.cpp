...
int binarySearchOnTree(ll p,ll l, ll r, ll k){
    if (l==r)
    {
        return l;
    }
    ...
    ll middle = mid(l, r);
    if (st[leftChild(p)].cnt >=k)
    {
        return binarySearchOnTree(leftChild(p),l,middle,k);
    }else {
        return binarySearchOnTree(rightChild(p),middle+1,r,k-st[leftChild(p)].cnt);
    }
...
int main(){
    ...
    F(i, 0, q){
        ...
        queries.push_back(make_pair(c, temp));
    }
    ...
    for(set<int>::iterator itr = nos.begin(); itr != nos.end(); ++itr){
        compression[*itr] = n++;
        tempnum.push_back(*itr);
    }

    build(1, 0, n-1);
    F(i, 0, q) {
        char c = queries[i].first ;
        int temps = queries[i].second;
        if (c == 'I')
        {
            update(1,0,n-1,compression[temps],1);
        }else if (c == 'D')
        {
            update(1, 0, n-1, compression[temps], 0);   
        }else if (c == 'K')
        {   
            if (temps<=0 || st[1].cnt<temps)
            {
               cout<<"invalid\n";
               continue; 
            }
            int value = binarySearchOnTree(1,0,n-1,temps);
            cout<<tempnum[value]<<endl;
               
        }else {
            if (compression[temps]==0)
            {
                cout<<0<<endl;
                continue;
            }
            cout<<query(1, 0, n-1, 0, compression[temps]-1).cnt<<endl;
        }
    }
